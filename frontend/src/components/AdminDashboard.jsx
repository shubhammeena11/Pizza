...
function AdminDashboard() {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      ...
      try {
        ...
        console.error(err);
        setError("Unable to load orders. Please check your login or try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="mx-4 sm:mx-6 lg:mx-20 my-8 rounded-3xl bg-white shadow-xl p-8">
      ...
      <p className="mt-2 text-gray-600">Only administrators can view customer orders and order details here.</p>

      {loading ? (
        <div className="mt-8 text-gray-600">Loading orders…</div>
      ) : error ? (
        <div className="mt-8 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-700">{error}</div>
      ) : orders.length === 0 ? (
        <div className="mt-8 text-gray-600">No orders have been placed yet.</div>
      ) : (
            ...
            <div key={order._id} className="rounded-3xl border border-gray-200 p-6 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-gray-900">{order.user?.name || "Unknown user"}</h2>
